...
def resample_series(x, y: pd.Series, rule: str, agg: str):
    ...
    try:
        tmp = pd.DataFrame(
            {"x": x.values, "y": pd.to_numeric(y, errors="coerce").values}
        ).set_index("x")
        grouped = tmp["y"].resample(rule)
        resampled = getattr(grouped, agg)()
    except (ValueError, KeyError):
        # e.g. a frequency alias unsupported by the installed pandas version —
        # fail soft instead of crashing the whole dashboard.
        return x, y, False
    return pd.Series(resampled.index), resampled.reset_index(drop=True), True


def _x_to_numeric(x) -> np.ndarray:
    """Convert a datetime or numeric x-axis into a plain float array for curve
    fitting.

    For datetimes we deliberately return *elapsed seconds since the first
    sample* rather than the raw epoch integer. Raw epoch values are huge
    (~1e15-1e18, and the exact magnitude even depends on pandas' internal
    datetime64 resolution, which varies between ns/us/ms across pandas
    versions and how the CSV was parsed). Feeding numbers that large into
    np.polyfit makes the Vandermonde matrix ill-conditioned for degree >= 2
    fits and overflows exp()/log() entirely for the Exponential/Logarithmic
    fits — Linear (degree 1) is simple enough to mostly survive it, which is
    why only the Linear trend used to render correctly.
    """
    x = pd.Series(x)
    if pd.api.types.is_datetime64_any_dtype(x):
        ...
        t0 = valid.iloc[0]
        return (x - t0).dt.total_seconds().to_numpy(dtype="float64")
...
def fit_trend(x, y: pd.Series, kind: str):
    ...
    x is always converted through _x_to_numeric() first, so for a datetime
    axis this is "seconds elapsed since the first sample" rather than a raw
    epoch number -- see that function's docstring for why the old raw-epoch
    version silently broke every fit except Linear.
    """
    is_dt = pd.api.types.is_datetime64_any_dtype(pd.Series(x))
    ...
    x_label_sym = "t" if is_dt else "x"
    try:
        if kind == "Linear":
            c = np.polyfit(xv, yv, 1)
            y_fit = np.polyval(c, x_num)
            eq = f"y = {c[0]:.4g}·{x_label_sym} + {c[1]:.4g}"

        elif kind == "Quadratic":
            c = np.polyfit(xv, yv, 2)
            y_fit = np.polyval(c, x_num)
            eq = f"y = {c[0]:.4g}·{x_label_sym}² + {c[1]:.4g}·{x_label_sym} + {c[2]:.4g}"
            ...
            shift_txt = f"+{round(shift, 2)}" if shift else ""
            eq = f"y = {c[0]:.4g}·ln({x_label_sym}{shift_txt}) + {c[1]:.4g}"
            ...
            eq = f"y = {np.exp(c[1]):.4g}·e^({c[0]:.4g}·{x_label_sym})"
    ...
    # Degenerate fit (e.g. exp() overflow on pathological data) — treat the
    # same as "couldn't fit" instead of handing the chart an inf/NaN line.
    if not np.all(np.isfinite(y_fit[mask])):
        return None
    ...
    if is_dt:
        eq += "  (t = seconds elapsed since first sample)"
    return y_fit, eq, r2
